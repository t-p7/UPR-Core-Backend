...
router.post("/insert/spreadsheet", async function (req, res) {
	...
	// console.log("received inforamtion", information)
	try {
		for (let i = 0; i < information.length; i++) {
			...
			height = (information[i].Dimensions.match(/\d+(?=H)/) || [])[0];
			width = (information[i].Dimensions.match(/\d+(?=W)/) || [])[0];
			depth = (information[i].Dimensions.match(/\d+(?=D)/) || [])[0];
	
			// console.log('Weight:', information[i].Weight); // Check what value is being passed
			...
			// --- Regions and Service Region Pricing ---
			...
			// Insert new regions or fetch existing ones
			for (let j = 0; j < information[i].regions.length; j++) {
				const regionObj = JSON.parse(information[i].regions[j]);
				let region_check = await queries.Search(region.TableName, [regionObj.box1]);
	
				if (!region_check) {
					await queries.Insert(region.TableName, ["RegionName"], [regionObj.box1]);
					region_check = await queries.Search(region.TableName, [regionObj.box1]);
				}
	
				region_id_list.push(region_check[0].RegionID); // Add the RegionID to the list
			}
	
			// Insert into Delivery & Service Region Pricing table
			for (let j = 0; j < information[i].regions.length; j++) {
				try {
					const regionObj = JSON.parse(information[i].regions[j]);
					const sanitizedBox3 = regionObj.box3.replace(/[^\d.-]/g, ''); // Remove junk characters
					const box3Value = await queries.GetNumber(sanitizedBox3); // Cleaned price value
	
					if (!isNaN(box3Value)) {
						// console.log(`Inserting into ${serviceregion.TableName}`, {
						// 	region_id: region_id_list[j],
						// 	box2: regionObj.box2,
						// 	box3: box3Value,
						// 	furniture_id
						// });
						...
						console.error(`Error with numeric value in box3: ${regionObj.box3}`);
					}
				} catch (error) {
					console.error(`Error parsing region at index ${j} for Delivery & Service Pricing: ${error.message}`);
				}
			}
