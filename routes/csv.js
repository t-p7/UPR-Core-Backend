...
router.post("/insert/spreadsheet", async function (req, res) {
	...
	for (let i = 0; i < information.length; i++) {
		...
		tier_check = await queries.Search(tier.TableName, [await queries.GetNumber(req.body[i].Tier1_Price),
			await queries.GetInt(req.body[i].Tier1_Quantity), await queries.GetNumber(req.body[i].Tier2_Price), 
			await queries.GetInt(req.body[i].Tier2_Quantity), await queries.GetNumber(req.body[i].Tier3_Price), 
			await queries.GetInt(req.body[i].Tier3_Quantity), await queries.GetNumber(req.body[i].Tier4_Price),
			await queries.GetInt(req.body[i].Tier4_Quantity), await queries.GetNumber(req.body[i].Tier5_Price), 
			await queries.GetInt(req.body[i].Tier5_Quantity)
			]);

		if (tier_check === null) {
			await queries.Insert(tier.TableName, tier.Columns, [tier_id,
				await queries.GetNumber(req.body[i].Tier1_Price),
				await queries.GetInt(req.body[i].Tier1_Quantity), await queries.GetNumber(req.body[i].Tier2_Price), 
				await queries.GetInt(req.body[i].Tier2_Quantity), await queries.GetNumber(req.body[i].Tier3_Price), 
				await queries.GetInt(req.body[i].Tier3_Quantity), await queries.GetNumber(req.body[i].Tier4_Price),
				await queries.GetInt(req.body[i].Tier4_Quantity), await queries.GetNumber(req.body[i].Tier5_Price), 
		...
		height = req.body[i].Dimensions.match(/\d+(?=H)/);
		width = req.body[i].Dimensions.match(/\d+(?=W)/);
		depth = req.body[i].Dimensions.match(/\d+(?=D)/);

		details_check = await queries.Search(details.TableName, [colours_id,
			req.body[i].Assembly_Required, await queries.GetNumber(height), await queries.GetNumber(width),
		...
		if (details_check === null) {
			await queries.Insert(details.TableName, details.Columns, [details_id,
				...
				req.body[i].Stackable, req.body[i].Adjustability, req.body[i].Ergonomic,
				req.body[i].Mechanism, req.body[i].Lumbar_Support, req.body[i].Compatible_With,
				req.body[i].Castors, await queries.GetNumber(req.body[i].Lifting_Capacity),
				await queries.GetNumber(req.body[i].Max_Load_Weight),
				req.body[i].Default_Warranty, aunzcode_id, accredit_id, req.body[i].Test_Certificate_Expiry]);
		}
		...
		furniture_check = await queries.Search(furniture.TableName, [req.body[i].ProductCode,
			...
			req.body[i].Long_Description, await queries.GetNumber(req.body[i].Unit_Price), await queries.GetNumber(req.body[i].MSRP_Price),
		...
		await queries.Insert(external_links.TableName, external_links.Columns,
			[external_links.Count, req.body[i].Thumbnail, req.body[i].PrimaryImage,
			req.body[i].Additional_Image, req.body[i].Additional_Image2, req.body[i].Additional_Image3, 
			req.body[i].Additional_Image4, req.body[i].Additional_Image5, req.body[i].Further_Document1,
			req.body[i].Further_Document1Label, req.body[i].Further_Document2,
			req.body[i].Further_Document2Label, furniture_id]);
